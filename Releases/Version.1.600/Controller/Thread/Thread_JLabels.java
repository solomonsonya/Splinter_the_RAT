...
Date dateTime;// = new Date();//get the current date, time, timezone  //  @jve:decl-index=0:
	...
	public boolean updateJLabels()
	{
		try
		{
			jlblCurrentHeap.setText("" + Thread_JLabels.getFormattedFileSize_String(Runtime.getRuntime().totalMemory()));
			jlblAvailableHeap.setText("" + Thread_JLabels.getFormattedFileSize_String(Runtime.getRuntime().freeMemory()));
			jlblMaxHeap.setText("" + Thread_JLabels.getFormattedFileSize_String(Runtime.getRuntime().maxMemory()));
			...
			jlblZuluTime.setText("" + Thread_JLabels.getZuluTimeStamp_Without_Date());
			
			try
			{
				this.jlblConsumedHeap.setText("" + Thread_JLabels.getFormattedFileSize_String(Math.abs(Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) ));
			}
			catch(Exception ee)
			{
