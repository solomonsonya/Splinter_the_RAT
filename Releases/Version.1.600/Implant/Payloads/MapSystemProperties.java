...
public class MapSystemProperties 
{
	...
	public boolean determineIfThisIsWindows()
	{
		try
		{
			...
			if(thisIsWin)
				MapSystemProperties.thisIsUNIXSystem = false; 
			
			else//it is a unix system
				MapSystemProperties.thisIsUNIXSystem = true;
		...
		catch(Exception e)
		{
			...
			MapSystemProperties.thisIsAWindowsSystem = false;
			MapSystemProperties.thisIsUNIXSystem = false; 
		...
		try
		{
			...
			try
			{
				...
				//these are tests we used, they work well also
					...
					strLaunchPath = strLaunchPath.replaceAll("%20"," ");
					strLaunchPath = strLaunchPath.replaceAll("/",Driver.fileSeperator);
					...
					try
					{
						...
						{
							MapSystemProperties.strLaunchBinary = strLaunchPath.substring(strLaunchPath.lastIndexOf(Driver.fileSeperator)+1);
	...
	public boolean mapCurrentWorkingDirectory(boolean thisIsWindows)
	{
		...
		{
			if(thisIsWindows)
				CurrentWorkingDirectory = (System.getProperty("user.dir")).replaceAll("\\", "/");
	...
	public boolean mapUserHomeDirectory(boolean thisIsWindows)
	{
		...
		{
			if(thisIsWindows)
				UserHomeDirectory = (System.getProperty("user.home")).replaceAll("\\", "/");
	...
	public boolean mapUserDesktop(boolean thisIsWindows)
	{
		try
		{
			if(thisIsWindows)
			{
				UserDesktop = (System.getProperty("user.home") + "/Desktop").replaceAll("\\", "/");				
			}
	...
	{
		return MapSystemProperties.thisIsAWindowsSystem;
	}
	...
	{
		return MapSystemProperties.thisIsUNIXSystem;
	}	
